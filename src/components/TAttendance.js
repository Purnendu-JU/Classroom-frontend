...
const TAttendance = () => {
  ...
  const fetchAttendanceRecords = async () => {
    try {
      const res = await fetch('https://classroom-backend-uv6g.onrender.com/api/auth/getattendance', {
        method: 'GET',
        headers: {
  ...
  const handleGenerateCode = async () => {
    try {
      const res = await fetch('https://classroom-backend-uv6g.onrender.com/api/auth/generateattendance', {
        method: 'POST',
        headers: {
  ...
  const handleDisableCode = async () => {
    ...
    try {
      const res = await fetch('https://classroom-backend-uv6g.onrender.com/api/auth/disablecode', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'auth-token': localStorage.getItem('token'),
