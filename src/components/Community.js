...
const Community = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const response = await fetch('https://classroom-backend-one.vercel.app/api/auth/getuser', {
          method: 'POST',
          headers: {
    ...
    const fetchMessages = async () => {
      try {
        const response = await fetch('https://classroom-backend-one.vercel.app/api/auth/getcommunity', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'auth-token': localStorage.getItem('token'),
  ...
  const handleSendMessage = async () => {
    if (user && message.trim() !== '') {
      try {
        const response = await fetch('https://classroom-backend-one.vercel.app/api/auth/postcommunity', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'auth-token': localStorage.getItem('token'),
