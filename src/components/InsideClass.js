  ...
  useEffect(() => {
    const fetchRoleAndClass = async () => {
      try {
        ...
        if (token && classToken) {
          const resRole = await fetch('https://classroom-backend-uv6g.onrender.com/api/auth/getrole', {
            headers: {
              'auth-token': token,
              'auth': classToken
            },
          ...
          const resClass = await fetch('https://classroom-backend-uv6g.onrender.com/api/auth/getclass', {
            headers: {
              'auth': classToken
            },
          });
